...
def get_client_agent_conversations(data_folder, input_file, client_file, agent_file, conv_file):
    ...
    for i, line in enumerate(input_file):
        ...
        if i > 1:
            ...
            if client_id != '' and (agent_id != '' or external_id != ''):
    ...
    data = zip(conv_id, conv_position, conv_length, range(len(conv_length)))
    df = pd.DataFrame(data, columns=['convID', 'convPos', 'convLen', 'linePos'])
