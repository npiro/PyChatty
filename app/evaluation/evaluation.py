...
class ChatEvaluator(object):
    ...
    def test(self, num_splits = 3, size = 100, window = 5, min_count = 3, workers = 7):
        """
    ...
    for i in range(len(evaSize)):
        for j in range(len(evaWin)):
            for k in range(len(evaMin)):
                cosSim, distIn, distOut = evaluator.test(size = evaSize[i], min_count = evaMin[k], window = evaWin[j] )
