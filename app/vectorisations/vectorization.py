...
def remove_NaNs_fromOR(matrixA, matrixB):
    """
    This functions takes two matrix of vectorised sentences (they need to have the same number of lines), 
    and return the matrix where the lines are NaN, in any of the two matrix. This is particularly useful
    for conducting the evaluations test.
    It returns as output:
    - matrixFilteredA: the matrix of vectorised words with the NaN lines removed.
    - matrixFilteredB: the matrix of vectorised words with the NaN lines removed.
    - correspondanceMap: an array which gives the correspondance map between corresponding lines
    between the matrices "matrix" and "matrixFiltered". correspondanceMatrix[i] returns j where maps
    line i of matrixFiltered corresponds to line j of matrix 
    
    """
    matrixNaNA = np.isnan( matrixA[:,0] ) 
    matrixNaNB = np.isnan( matrixB[:,0] ) 
    matrixNaN = matrixNaNA + matrixNaNB
    matrixFilteredA = matrixA[ ~ matrixNaN]    
    matrixFilteredB = matrixB[ ~ matrixNaN]  
    
    vectorRange=np.empty( len( matrixA[:,1] ) )
    for n in range( len(matrixA[:,1]) ):
        vectorRange[n] = n
    correspondanceMap = vectorRange [ ~ matrixNaN ] 
    return matrixFilteredA, matrixFilteredB, correspondanceMap.astype(int)
...
class Word2Vec(object):
    ...
    def __init__(self, filenames, folder = '.', load_from_file = False, size = 100, window = 5, min_count = 3, workers = 4):
        ...
        if not load_from_file:
            ...
            self.load(filenames, folder=folder)
    ...
    def load(self, filename, folder = '.'):
        ...
        self.model = gensim.models.Word2Vec.load(fn)
