...
class Sentiment(object):
    ...
    def plot_sentiment_for_chat(self, ax=None):
        """
        Plots the sentiment of the conversation (both client and agent) in
        a single plot.
        :param ax: [obj], optional matplotlib axis to plot to.
        :return plot or nothing if ax is defined.
        """
        if len(self.client_msgs) > 0 and len(self.agent_msgs) > 0:
            ...
            if ax is not None:
                df.plot(ax=ax, xticks=range(1, df.shape[0]+1))
                ax.set_xlabel("Message number")
                ax.set_ylabel("Sentiment")
            else:
                ax = df.plot(xticks=range(1, df.shape[0]+1))
                ax.set_xlabel("Message number")
                ax.set_ylabel("Sentiment")
                return ax
        else:
